...
Use Cases:
- Uploading reference images for image-to-video generation
- Any scenario requiring local files or base64-encoded data as model input

Usage:
    # Method 1: Upload from a file path
    python file_to_oss.py --file /path/to/image.jpg --model wan2.7-i2v
    
    # Method 2: Upload from base64 data
    python file_to_oss.py --base64 "<base64_data>" --model wan2.7-i2v
...
def upload_file_to_oss(api_key, model_name, file_path=None, 
                       base64_data=None, filename=None):
    """
    Upload a file to temporary OSS storage
    
    Args:
        api_key: Alibaba Cloud DashScope API Key
        model_name: Model name (e.g., wan2.7-i2v)
...
Example:
  ...
  python file_to_oss.py --base64 "<base64_data>" --model wan2.7-i2v
    ...
    parser.add_argument('--model', '-m', required=True, help='Model name (e.g., wan2.7-i2v)')
    ...
    if not args.file and not args.base64:
        print("❌ Error: Please provide one of --file or --base64")
